...
describe('ESTree AST Parser Tests', function() {
  ...
  describe('ES6 Stateless Functional Component Parsing Tests', function() {
    ...
    it('should return an object with correct name property', function() {
      expect(getStatelessFunctionalComponents(statelessFuncFixtures.statelessNested).name).to.equal('Foo');
    })

    it('should account for nested children components', function() {
      expect(getStatelessFunctionalComponents(statelessFuncFixtures.statelessNested).children[0].name).to.equal('Bar');
    })

    it('should return an object with correct props', function(){
      expect(getStatelessFunctionalComponents(statelessFuncFixtures.statelessNested).children[0].props.length).to.equal(1);
    })
  })
});